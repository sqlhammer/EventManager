...
- [ ] CONSTRUCTION: U4b Hub Competition — **fast-tracked; code complete on branch `unit/u4b-hub-competition`** (`admin/EventManager.Hub/Competition` orchestrating U1 engines: bracket lifecycle, mat-authority scoring, weigh-in resolution, finalization, disputes; 12 hub tests total). First unit under CS-1 (no ternaries). Awaiting merge this turn.
- [ ] CONSTRUCTION: U7, U5, U6 (per build order, each on its own branch)
- [ ] REFACTOR SUB-UNIT **R1 — Ternary Elimination** (added 2026-07-25 per user): replace all ternary conditional `?:` operators across U1–U4b with readable if/else / switch expressions / named locals, per coding-standards.md CS-1. Keep `??`/`?.`. Own branch `refactor/r1-ternary`; build+tests green before merge. New code from 2026-07-25 onward already avoids `?:`.
