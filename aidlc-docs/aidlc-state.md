...
- **Current Stage**: CONSTRUCTION — **U7 merged to main 2026-07-25 (merge 0b51346). Next units: U5 Judge + U6 Check-In (MAUI apps).** U1 merged 2026-07-24; U2+U8+U3+U4a+U4b+U7 + refactor R1 merged 2026-07-25. 85 tests green (shared 42, backend 26, admin 17). Coding standard CS-1 (no ternaries) active.
...
- [ ] CONSTRUCTION: U7 Offline Resilience — **fast-tracked; code complete on branch `unit/u7-offline-resilience`** (`admin/EventManager.Hub/Resilience`: ReplicationClient + transport seam, BackupService/RecoveryService, integrating U1 ReplicationProtocol + U2 LocalEventQueue; 17 hub tests incl. zero-internet PBT). Awaiting merge this turn.
- [ ] CONSTRUCTION: U5 Judge — **fast-tracked; code complete on branch `unit/u5-judge`** (`judge/EventManager.Judge.Core` + **compiling MAUI Windows head** + 6 tests; durable-before-ack scoring, mat queue, cross-mat read-only, focus mode). MAUI workloads installed; Windows head builds, Android/iOS/Mac deferred (no JDK/SDK/Mac). Awaiting merge this turn.
- [ ] CONSTRUCTION: U6 Check-In — **NEXT** (branch `unit/u6-checkin`); check-in + weigh-in range validation (US-306/307), same app-core + Windows-head pattern.
