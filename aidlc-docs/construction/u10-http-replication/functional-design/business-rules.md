...
| **BR-REPL-3** | The cloud persists only a salted hash of the key, using the hashing already in `Services/Security.cs`. A reader of the cloud database cannot recover a usable credential. | U10-FR-2, SECURITY-12 |
