...
| 5 | `admin/.../ReplicationClient.cs` | **Amended 2026-07-27 (AD-Q4=B)**: retry classification (D-U10-03) **plus** the replication schedule — append-signal channel consumer, drain timer, close-out flush, `BackgroundService` lifetime, and `IServiceScopeFactory` for the scoped store (CL-1=A) | Deliberately separated from step 4 — it is the only edit to **merged U7 code**. This step was originally scoped to retry classification alone; AD-Q4=B widened it, so the gate that the 17 existing admin tests stay green is now the primary protection for the flagship offline guarantee, not a formality |
