...
## Notes
- **Auth model:** the collection sends `Authorization: Bearer {{accessToken}}` by default. Anonymous cloud endpoints (register / confirm / login / refresh / health) and the entire hub folder override to **No Auth** — the hub's LAN surface is unauthenticated by design (LAN trust model). `{{accessToken}}` starts empty and is populated by the **Login** request's test script — so you must run Login (as a valid, non-deleted account) before any authenticated call, including *Delete my account*.
- **Variable scopes:** script-captured values (`accessToken`, `eventId`, `divisionId`, `athleteId`, `registrationId`, `enrollmentToken`, `deviceId`, `disputeId`) live in **collection scope** and are deliberately NOT in the environment file — an environment variable outranks a collection one, so an empty env copy would shadow the captured value and every authenticated call would 401. The environment holds only config + hand-entered ids (base URLs, credentials, `matchId`, `winnerId`, `coOrganizerAccountId`, `confirmToken`).
